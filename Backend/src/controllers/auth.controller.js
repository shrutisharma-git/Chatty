...
export async function signup(req, res) {
  ...
  try {
    ...
    if (existingUser) {
      return res.status(400).json({ message: "Email already exists. Please try another." });
    }
    ...
    try {
      ...
      console.log("Stream user creation error:", error.message);
    }
    ...
    res.cookie("jwt", token, {
      ...
      sameSite: "none",       // ✅ allow cross-origin cookie for Vercel + Render
    });
...
export async function login(req, res) {
  try {
    ...
    // ✅ FIXED COOKIE SETTINGS (same as signup)
    res.cookie("jwt", token, {
      maxAge: 7 * 24 * 60 * 60 * 1000,
      httpOnly: true,
      secure: true,
...
export async function onboard(req, res) {
  try {
    ...
    try {
      ...
      console.log("Stream update error:", error.message);
    }
